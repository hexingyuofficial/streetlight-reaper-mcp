...
3. 从 `errors.ts` 生成 Lua 错误码常量文件(如 `packs/core/error_codes.lua`),并加一个 grep
   级检查:handler 里出现的错误码字符串必须在全集内。
...
- **不要试图从 TS 生成整个 manifest.lua**:handler 函数体是手写的,生成会覆盖逻辑。只生成/
  校验**元数据对齐**。
- 错误码迁移触及多个 handler(现用字符串字面量);改为引用常量是机械活,但要一次到位 + grep
  检查兜底,否则漂移依旧。

### 验证
- 故意把某模板 TS 的 `entity_kind` 改得与 Lua 不一致 → `check-manifest-alignment.mjs` 报错。
- handler 里写一个不存在的错误码 → grep 检查报错。
- `list_templates` 输出含每模板 example + expectedDelta 摘要。

### DoD
模板元数据 TS↔Lua 不一致时 CI 必红;错误码有单一来源 + 检查;`list_templates` 富化。
