...
Slices 14 and 15 add bridge-level retry safety on top of this file queue:
...
`render_region` is deferred internally, but Slice 15 stores its terminal
inner envelope after the deferred slot closes, so same-key render
retries replay instead of starting a second render. Replay does not
re-check the WAV on disk.
