...
describe("Lua bridge structure", () => {
  ...
  it("wires Slice 25 item_audio_analyze as a bounded read-only artifact template", async () => {
    ...
    expect(analysisLua).toMatch(/cursor = window\.local_start/);
    expect(analysisLua).toMatch(/while cursor < window\.local_end - EPSILON/);
    expect(analysisLua).not.toMatch(/window\.item_position \+ local_start/);
    expect(analysisLua).not.toMatch(/cursor = window\.project_start/);
    ...
    expect(analysisLua).toMatch(/MAX_TRANSIENTS = 200/);
    expect(analysisLua).toMatch(/TRANSIENT_FRAME_SAMPLES = 512/);
    expect(analysisLua).toMatch(/TRANSIENT_MIN_GAP_SECONDS = 0\.05/);
    expect(analysisLua).toMatch(/TRANSIENT_RISE_THRESHOLD_DB = 10/);
    expect(analysisLua).toMatch(/TRANSIENT_THRESHOLD_FLOOR_DBFS = -60/);
    expect(analysisLua).toMatch(/MAX_LOOP_CANDIDATES = 5/);
    expect(analysisLua).toMatch(/LOOP_MIN_DURATION_SECONDS = 0\.25/);
    expect(analysisLua).toMatch(/LOOP_MAX_DURATION_SECONDS = 8\.0/);
    expect(analysisLua).toMatch(/LOOP_MIN_TRANSIENT_INDEX_GAP = 1/);
    expect(analysisLua).toMatch(/LOOP_MAX_PAIRS_CONSIDERED = 4096/);
    expect(analysisLua).toMatch(/LOOP_SILENCE_MARGIN_SECONDS = 0\.04/);
    expect(analysisLua).toMatch(/LOOP_PEAK_CONTINUITY_MAX_DB = 18/);
    expect(analysisLua).toMatch(/CLICK_RISK_WINDOW_MS = 12/);
    expect(analysisLua).toMatch(/CLICK_RISK_MIN_DURATION_SECONDS = 0\.05/);
    expect(analysisLua).toMatch(/CLICK_RISK_MAX_DURATION_SECONDS = 8\.0/);
    expect(analysisLua).toMatch(/CLICK_RISK_LOW_THRESHOLD = 0\.33/);
    expect(analysisLua).toMatch(/CLICK_RISK_HIGH_THRESHOLD = 0\.66/);
    expect(analysisLua).toMatch(/CLICK_RISK_SAMPLE_DELTA_NORM = 0\.5/);
    expect(analysisLua).toMatch(/CLICK_RISK_HARD_DISCONTINUITY_DELTA = 0\.5/);
    expect(analysisLua).toMatch(/threshold_db = math\.max\(TRANSIENT_THRESHOLD_FLOOR_DBFS, peak_db - 36\)/);
    expect(analysisLua).toMatch(/algorithm_version = "transients_v1"/);
    expect(analysisLua).toMatch(/algorithm_version = "loop_candidates_v1"/);
    expect(analysisLua).toMatch(/algorithm_version = "click_risk_v1"/);
    expect(analysisLua).toMatch(/events = ctx\.json\.array\(transients\.events\)/);
    expect(analysisLua).toMatch(/candidates = ctx\.json\.array\(loop_candidates\.candidates\)/);
    expect(analysisLua).toMatch(/reason = "transient_pair_duration_peak_continuity"/);
    expect(analysisLua).toMatch(/features, "loop_candidates"/);
    expect(analysisLua).toMatch(/features, "click_risk"/);
    expect(analysisLua).toMatch(/score_direction = "higher_is_more_dangerous"/);
    expect(analysisLua).toMatch(/hard_discontinuity_delta = CLICK_RISK_HARD_DISCONTINUITY_DELTA/);
    expect(analysisLua).toMatch(/risk_label = risk_label\(score\)/);
    expect(analysisLua).toMatch(/source = "best_loop_candidate"/);
    expect(analysisLua).toMatch(/source = "user"/);
    expect(analysisLua).toMatch(/transient_threshold_floor_dbfs = TRANSIENT_THRESHOLD_FLOOR_DBFS/);
    expect(analysisLua).toMatch(/loop_max_pairs_considered = LOOP_MAX_PAIRS_CONSIDERED/);
    expect(analysisLua).toMatch(/click_risk_window_ms = CLICK_RISK_WINDOW_MS/);
    expect(analysisLua).toMatch(/threshold_floor_dbfs = transients\.threshold_floor_dbfs/);
    expect(analysisLua).toMatch(/RMS dBFS, not LUFS/);
    expect(analysisLua).toMatch(/Sample peak, not true peak/);
    expect(analysisLua).toMatch(/transients are heuristic onset candidates/);
    expect(analysisLua).toMatch(/loop_candidates are heuristic intervals/);
    expect(analysisLua).toMatch(/click_risk is a heuristic boundary score/);
    ...
    expect(analysisLua).not.toMatch(/scope = "click_risk"/);
    ...
    expect(analysisLua).not.toMatch(/D_FADEINLEN|D_FADEOUTLEN/);
